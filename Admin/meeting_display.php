...
function search_record()
 ...
 <div class="footer-top"></div>
        <form name="meeting" id="meeting" method="post" onSubmit="return search_record();">
        <div style=" width:800;overflow:auto">
    <h2 align="center">Meeting Form</h2>
    <hr /><br /><div align="right">
        <input type="text" name="keyword" id="keyword" value="<?php echo $keyword?>" >&nbsp;
        <input type="submit" value="Search" class="button" name="search">&nbsp;
        <input type="button" value="Show All" class="button" name="show all" onClick="window.location='meeting_display.php';">
        </div>
        </form>
...
<!----------------pagination second part-------------------->
    ...
    <!--------------------pagination sec part complete-------------->			
     ...
     <tr>
     	<td><input type="checkbox" name="chk[]" value="<?php echo $d['meet_id'];?>" class="check" /></td>
     	<td><?php echo $d['meet_id'];?></td>
     	<td><?php if(isset($d['meet_title'])) echo $d['meet_title'];?></td>
      	<td><?php if(isset($d['meet_invit'])) echo htmlentities($d['meet_invit']);?></td>
     	<td><?php if(isset($d['meet_place'])) echo $d['meet_place'];?></td>
     	<td><?php if(isset($d['meet_time'])) echo $d['meet_time'];?></td>
     	<td><?php if(isset($d['meet_date'])) echo $d['meet_date'];?></td>

       <td><a href="?meet_id=<?php echo $d['meet_id'];?>&amp;action=delete" class="font"><img src="style/images/delete.png" onclick="return confirm('Are You Sure Want To Delete ?');" /></a></td>
       <td><a href="meeting_edit.php?meet_id=<?php echo $d['meet_id'];?>&amp;action=edit" class="font"><img src="style/images/edit.png" /></a></td>
      </tr>
          ...
          <?php }  ?>                  
           ...
           <tr>
       		<td colspan="9" align="center">
                ...
                <a href="meeting_display.php"><input type="submit" name="muldelete" value="Delete" class="button" onclick="return confirm('Are You Sure Want To Delete ?');" /></a></td>
    </tr>  
    </table>	
</center> 
</div>
...
<?php
	if(isset($_REQUEST['meet_id'],$_REQUEST['action'])&&$_REQUEST['action']=='delete')
{
	//echo "hi";

	//echo $_GET['admin_id'];
	$q="delete from meeting where meet_id='$_GET[meet_id]'";
...
<?php
	if(isset($_REQUEST['muldelete']) && $_REQUEST['muldelete']=='Delete')
	{
		...
		if($cnt)
		{
			...
			{
				echo "<br>";
				echo $q="delete from meeting where meet_id=$id[$i]";	
				mysql_query($q) or die(mysql_error());
			}
			echo "<script>alert('record deleted');
			window.location='meeting_display.php';
