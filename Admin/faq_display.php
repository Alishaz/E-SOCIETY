...
<?php
	...
	if(isset($_POST['search']) && $_POST['search']='Search')
	{
		...
		$src=" WHERE faq_que LIKE '%$keyword%' OR  faq_ans LIKE '%$keyword%'";	
	}
?>
</head>
<body>
...
function search_record()
 ...
 <div class="footer-top"></div>
    <form name="faq" id="faq" method="post" onSubmit="return search_record();">
    <div style=" width:800;overflow:auto">
    <h2 align="center">FAQ Form</h2>
    <hr /><br /><div align="right">
    <input type="text" name="keyword" id="keyword" value="<?php echo $keyword?>" >&nbsp;
        <input type="submit" value="Search" class="button"  name="search">&nbsp;
        <input type="button" value="Show All" class="button" name="show all" onClick="window.location='faq_display.php';">
       </div>
        </form>
...
<!----------------pagination second part-------------------->
    ...
    <!--------------------pagination sec part complete-------------->			
     ...
     <tr>
     	<td><input type="checkbox" name="chk[]" value="<?php echo $d['faq_id'];?>" class="check" /></td>
        <td><?php echo $d['faq_id'];?></td>
     	<td><?php if(isset($d['faq_que'])) echo $d['faq_que'];?></td>
      	<td><?php if(isset($d['faq_ans'])) echo $d['faq_ans'];?></td>
        <td><a href="?faq_id=<?php echo $d['faq_id'];?>&amp;action=delete" class="font"><img src="style/images/delete.png" onclick="return confirm('Are You Sure Want To Delete ?');" /></a></td>
		<td><a href="faq_edit.php?faq_id=<?php echo $d['faq_id'];?>&amp;action=edit" class="font"><img src="style/images/edit.png" /></a></td>
      </tr>
     ...
     <tr>
       		<td colspan="6" align="center">
                ...
                <a href="faq_display.php"><input type="submit" name="muldelete" value="Delete" class="button" onclick="return confirm('Are You Sure Want To Delete ?');" /></a></td>
       </tr>
...
?>
 ...
 <?php 
if(isset($_REQUEST['faq_id'],$_REQUEST['action'])&&$_REQUEST['action']=='delete')

{
	echo "hiii";
	
	$q="delete from faq where faq_id='$_GET[faq_id]'";
...
<?php
	if(isset($_REQUEST['muldelete']) && $_REQUEST['muldelete']=='Delete')
	{
		...
		if($cnt)
		{
			...
			{
				echo "<br>";
				echo $q="delete from faq where faq_id=$id[$i]";	
				mysql_query($q) or die(mysql_error());
			}
			echo "<script>alert('record deleted');
			window.location='faq_display.php';
